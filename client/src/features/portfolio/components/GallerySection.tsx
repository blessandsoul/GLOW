...
export function GallerySection({
    ...
    const handleAdd = async (data: PortfolioItemFormData): Promise<void> => {
        await onAdd(data);
    };
            ...
            <Drawer open={pickerOpen} onOpenChange={setPickerOpen}>
                <DrawerContent className="px-4 pb-8 max-h-[85vh]">
                    <DrawerHeader className="px-0">
                        <DrawerTitle>{t('portfolio.add_from_work')}</DrawerTitle>
                        {availableToAdd > 0 && (
                            <p className="text-sm text-muted-foreground">
                                {availableToAdd} {t('portfolio.gallery_available')}
                            </p>
                        )}
                    </DrawerHeader>
