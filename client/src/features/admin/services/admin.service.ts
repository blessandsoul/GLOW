...
class AdminService {
    ...
    async getVariablePoolStatus(): Promise<VariablePoolStatus> {
        const { data } = await apiClient.get<ApiResponse<VariablePoolStatus>>(
            API_ENDPOINTS.FILTERS.VARIABLE_POOL_STATUS,
        );
        return data.data;
    }
