...
UPDATE `master_profiles` SET `city` = 'tbilisi' WHERE `city` IN ('თბილისი', 'Тбилиси', 'Tbilisi', 'TBILISI', 'Tbilis');
...
UPDATE `master_profiles` SET `city` = 'senaki' WHERE `city` IN ('სენაკი', 'Сენაки', 'Senaki');
UPDATE `master_profiles` SET `city` = 'samtredia' WHERE `city` IN ('სამტრედია', 'Самтредиа', 'Samtredia');
UPDATE `master_profiles` SET `city` = 'marneuli' WHERE `city` IN ('მარნეული', 'Марნეული', 'Marneuli');
